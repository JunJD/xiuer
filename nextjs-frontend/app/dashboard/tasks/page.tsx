...
export default async function TasksPage() {
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">Error: {tasksData?.message as string}</p>
        </div>
      </div>
    );
  }
  ...
  // 统计数据（如果获取失败则使用默认值）
  const stats = (statsData && 'total_tasks' in statsData) ? statsData : {
    ...
    <div className="h-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white p-4 rounded-lg shadow">
          ...
          <p className="text-2xl font-bold text-blue-600">{stats.running_tasks}</p>
      ...
      <section className="p-6 bg-white rounded-lg shadow-lg mt-8">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-gray-500">查看和管理所有爬取任务的详细信息</p>
          </div>
